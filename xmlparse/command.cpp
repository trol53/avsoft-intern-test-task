...
void Create_Employment::execute(std::map <Department, std::vector<Employment>>& dict){
    ...
    if (iter != dict.end()){
        ...
        if (is_exist(iter->second, employ)){
            std::cout << "already exist\n";
            return;
        }
        last_dep = iter->first;
...
bool Create_Employment::is_exist(std::vector<Employment>& employs, Employment& employ){
    for (auto tmp : employs){
        if (tmp == employ)
            return true;
    }
    return false;
...
void Change_Employment::input_change(Employment& employ){
    ...
    if (flag == "1"){
        ...
        std::getline(std::cin, tmp);
        int sal = std::atoi(tmp.c_str());
        if (sal <= 0){
            std::cout << "wrong input salary\n";
            return;
        }
